...
uploaded_file = st.file_uploader("Upload our resume (PDF)", type = ["pdf"])

if uploaded_file:
    with st.spinner("Extracting text from your resume..."):
        resume_text=extract_text_from_pdf(uploaded_file)

    with st.spinner("Summarizing yoru resume..."):
        summary = ask_llm(f"Summarize this resume highlighting the skills, education, and experience: \n\n{resume_text}", max_tokens=500)
    ...
    with st.spinner("Creating future roadmaps..."):
        roadmap = ask_llm(f"Based on this resume, suggest a future roadmap to improve this person's career prospects (skills to learn, certification needed, industry exposure): \n\n{resume_text}", max_tokens=500)
